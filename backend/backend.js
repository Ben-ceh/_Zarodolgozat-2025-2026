...
// Reakció (Lájk) kezelése: Ha van, törli, ha nincs, felviszi
app.post('/reakcioKezeles', (req, res) => {
    const { bejegyzes_id, felhasznalo_id, reakcio } = req.body;

    // 1. Megnézzük, létezik-e már ez a lájk
    const checkSql = "SELECT * FROM reakciok WHERE bejegyzes_id = ? AND felhasznalo_id = ?";
    
    pool.query(checkSql, [bejegyzes_id, felhasznalo_id], (err, results) => {
        if (err) return res.status(500).json({ error: "DB hiba" });

        if (results.length > 0) {
            // 2. Ha már létezik -> TÖRLÉS (Visszavonás)
            const deleteSql = "DELETE FROM reakciok WHERE bejegyzes_id = ? AND felhasznalo_id = ?";
            pool.query(deleteSql, [bejegyzes_id, felhasznalo_id], (err) => {
                if (err) return res.status(500).json({ error: "Törlési hiba" });
                return res.status(200).json({ status: "removed", message: "Lájk visszavonva" });
            });
        } else {
            // 3. Ha még nem létezik -> FELVITEL
            const insertSql = "INSERT INTO reakciok (reakciok_id, bejegyzes_id, felhasznalo_id, reakcio, letrehozva) VALUES (null, ?, ?, ?, NOW())";
            pool.query(insertSql, [bejegyzes_id, felhasznalo_id, reakcio], (err) => {
                if (err) return res.status(500).json({ error: "Felviteli hiba" });
                return res.status(200).json({ status: "added", message: "Lájk sikeres" });
            });
        }
    });
});
